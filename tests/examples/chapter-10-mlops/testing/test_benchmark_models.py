...
try:
    from chapter_10_mlops.testing.benchmark_models import ModelBenchmarker, run_model_benchmarks
except ImportError:
    ...
    class ModelBenchmarker:
        ...
        def benchmark_inference_speed(self, batch_sizes=[1, 2, 4, 8]):
            ...
            for batch_size in batch_sizes:
                ...
                base_latency = 50 + (batch_size * 15)  # Latency increases with batch size
                base_throughput = 120 - (batch_size * 8)  # Throughput decreases with larger batches
        ...
        def run_full_benchmark(self):
            ...
            try:
                ...
                benchmark_results["efficiency_metrics"] = {
                    "tokens_per_gb_gpu": batch_1_perf["avg_tokens_per_second"] / memory_usage["gpu_memory_gb"]["model_size"],
                    "latency_per_gb_model": batch_1_perf["avg_latency_ms"] / memory_usage["gpu_memory_gb"]["model_size"],
                    "memory_efficiency_score": (batch_1_perf["avg_tokens_per_second"] * 100) / memory_usage["gpu_memory_gb"]["peak"]
                }
        ...
        def compare_with_baseline(self, baseline_results):
            ...
            if current_results["status"] != "completed" or baseline_results["status"] != "completed":
            ...
            comparison = {
                ...
                "latency_change_percent": ((current_batch_1["avg_latency_ms"] - baseline_batch_1["avg_latency_ms"]) / baseline_batch_1["avg_latency_ms"]) * 100,
                "throughput_change_percent": ((current_batch_1["avg_tokens_per_second"] - baseline_batch_1["avg_tokens_per_second"]) / baseline_batch_1["avg_tokens_per_second"]) * 100,
                "memory_change_percent": ((current_memory["model_size"] - baseline_memory["model_size"]) / baseline_memory["model_size"]) * 100,
            ...
            if comparison["latency_change_percent"] > 10:
                ...
                comparison["recommendations"].append("Latency regression detected - investigate model or system changes")
            
            if comparison["throughput_change_percent"] < -10:
                comparison["performance_regression"] = True
                comparison["recommendations"].append("Throughput regression detected - check resource allocation")
            
            if comparison["memory_change_percent"] > 15:
                comparison["recommendations"].append("Memory usage increased significantly - review model size")
            
            if not comparison["recommendations"]:
                comparison["recommendations"].append("Performance maintained within acceptable range")
    ...
    def run_model_benchmarks():
        ...
        summary = {
            ...
            "successful_benchmarks": len([r for r in all_results if r["status"] == "completed"]),
            "failed_benchmarks": len([r for r in all_results if r["status"] == "failed"]),
        ...
        if len(successful_results) >= 2:
            ...
            best_latency = min(successful_results, key=lambda x: x["inference_speed"]["batch_1"]["avg_latency_ms"])
            best_throughput = max(successful_results, key=lambda x: x["inference_speed"]["batch_1"]["avg_tokens_per_second"])
            most_efficient = max(successful_results, key=lambda x: x["efficiency_metrics"]["memory_efficiency_score"])
            ...
            summary["comparative_analysis"] = {
                ...
                "performance_variance": {
                    "latency_range_ms": [
                        min(r["inference_speed"]["batch_1"]["avg_latency_ms"] for r in successful_results),
                        max(r["inference_speed"]["batch_1"]["avg_latency_ms"] for r in successful_results)
                    ],
                    "throughput_range_tokens_per_sec": [
                        min(r["inference_speed"]["batch_1"]["avg_tokens_per_second"] for r in successful_results),
                        max(r["inference_speed"]["batch_1"]["avg_tokens_per_second"] for r in successful_results)
                    ]
                }
            }
...
class TestModelBenchmarker:
    ...
    def test_inference_speed_benchmarking(self, benchmarker_7b):
        ...
        for batch_size in default_batch_sizes:
            ...
            required_metrics = [
                ...
                "avg_tokens_per_second", "throughput_requests_per_second"
            ]
            ...
            assert batch_result["min_latency_ms"] <= batch_result["avg_latency_ms"] <= batch_result["max_latency_ms"]
    ...
    def test_inference_speed_batch_scaling(self, benchmarker_7b):
        ...
        assert max(throughputs) > min(throughputs), "Throughput should vary with batch size"
    ...
    def test_memory_usage_benchmarking(self, benchmarker_7b):
        ...
        # Verify logical ordering
        assert gpu_memory["initial"] <= gpu_memory["after_loading"] <= gpu_memory["peak"]
    ...
    def test_model_size_estimation(self, benchmarker_7b, benchmarker_13b):
        ...
        assert 10 <= gpu_7b <= 20, f"7B model GPU usage {gpu_7b:.1f}GB outside expected range"
        assert 20 <= gpu_13b <= 35, f"13B model GPU usage {gpu_13b:.1f}GB outside expected range"
    ...
    def test_full_benchmark_execution(self, benchmarker_7b):
        ...
        required_top_level = [
            "model_name", "model_path", "timestamp", "system_info", "status"
        ]
    ...
    def test_benchmark_comparison(self, benchmarker_7b):
        ...
        required_comparison_fields = [
            "model_name", "comparison_timestamp", "latency_change_percent",
            "throughput_change_percent", "memory_change_percent",
            "performance_regression", "recommendations"
        ]
    ...
    @pytest.mark.parametrize("custom_batch_sizes", [
        [1, 4],
        [2, 8, 16],
        [1, 2, 4, 8, 16, 32],
    ])
    ...
    def test_performance_regression_detection(self, benchmarker_7b):
        ...
        with patch.object(benchmarker_7b, 'run_full_benchmark') as mock_benchmark:
            ...
            assert any("regression" in rec.lower() for rec in comparison["recommendations"])
    ...
    def test_run_model_benchmarks_function(self):
        ...
        required_summary_fields = [
            "benchmark_run_timestamp", "models_benchmarked",
            "successful_benchmarks", "failed_benchmarks"
        ]
    ...
    def test_comparative_analysis(self):
        ...
        if summary["successful_benchmarks"] >= 2:
            ...
            required_fields = [
                "best_latency_model", "best_throughput_model",
                "most_memory_efficient_model", "performance_variance"
            ]
            ...
            assert variance["throughput_range_tokens_per_sec"][0] <= variance["throughput_range_tokens_per_sec"][1]
    ...
    def test_efficiency_metrics_calculation(self, benchmarker_7b):
        ...
        if results["status"] == "completed":
            ...
            # Verify calculations
            expected_tokens_per_gb = batch_1_perf["avg_tokens_per_second"] / memory_usage["model_size"]
            assert abs(efficiency["tokens_per_gb_gpu"] - expected_tokens_per_gb) < 0.01
            
            expected_latency_per_gb = batch_1_perf["avg_latency_ms"] / memory_usage["model_size"]
            assert abs(efficiency["latency_per_gb_model"] - expected_latency_per_gb) < 0.01
