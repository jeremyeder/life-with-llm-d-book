    ...
    class LoadTester:
        ...
        async def load_test(self, total_requests: int, concurrent_requests: int):
            ...
            if successful_requests:
                ...
                tokens_per_request = [
                    r["tokens_generated"] for r in successful_requests
                ]
                ...
                metrics = {
                    ...
                    "throughput": {
                        "avg_tokens_per_request": (
                            statistics.mean(tokens_per_request)
            ...
            return {
                ...
                "latest_performance": {
                    ...
                    "throughput_tokens_per_sec": (
                        latest_result["throughput"]["total_tokens_generated"]
                        / (latest_result["latency_stats"].get("mean_ms", 1) / 1000)
                        if latest_result["latency_stats"]
                        else 0
                    ),
                },
...
class TestLoadTester:
    ...
    async def test_concurrent_load_scaling(self, load_tester):
        ...
        for concurrency in concurrency_levels:
            result = await load_tester.load_test(
                total_requests=15, concurrent_requests=concurrency
            )
            results.append(result)
    ...
    async def test_performance_thresholds(self, load_tester):
        ...
        if result["successful_requests"] > 0:
            ...
            ), f"Success rate {result['success_rate']:.2f} below threshold"

            # Latency should be reasonable for test environment
            latency_stats = result["latency_stats"]
            assert (
                latency_stats["p95_ms"] <= 1000
            ), f"P95 latency {latency_stats['p95_ms']:.0f}ms too high"
            assert (
                latency_stats["mean_ms"] <= 500
            ), f"Mean latency {latency_stats['mean_ms']:.0f}ms too high"

            # Token generation should be reasonable
            assert (
                result["throughput"]["avg_tokens_per_request"] >= 20
            ), "Token generation too low"
    ...
    async def test_error_handling(self, load_tester):
        ...
        result = await load_tester.load_test(
            total_requests=5, concurrent_requests=10
        )  # More concurrency than requests
    ...
    @pytest.mark.asyncio
    async def test_different_load_configurations(
        self, load_tester, total_requests, concurrent_requests
    ):
        ...
        assert (
            result["successful_requests"] + result["failed_requests"] == total_requests
        )
    ...
    async def test_load_test_consistency(self, load_tester):
        ...
        for _ in range(3):
            result = await load_tester.load_test(
                total_requests=12, concurrent_requests=4
            )
            results.append(result)
        ...
        if all(sr > 0 for sr in success_rates):
            ...
            assert (
                max_variance <= 0.3
            ), f"Success rate variance {max_variance:.2f} too high"
    ...
    async def test_latency_distribution_analysis(self, load_tester):
        ...
        if result["successful_requests"] > 0:
            ...
            assert (
                latency_stats["median_ms"] <= latency_stats["mean_ms"]
                or latency_stats["median_ms"] <= latency_stats["p95_ms"]
            )
    ...
    async def test_error_rate_analysis(self, load_tester):
        ...
        assert (
            0 <= error_rate <= 0.2
        ), f"Error rate {error_rate:.2%} outside expected range"
