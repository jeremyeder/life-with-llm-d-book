...
try:
    from chapter_10_mlops.istio_configs.experiment_manager import ExperimentManager, Experiment, run_model_comparison_experiment
except ImportError:
    ...
    class ExperimentManager:
        ...
        def create_experiment(self, experiment: Experiment) -> bool:
            ...
            try:
                ...
                self.k8s_client.create_namespaced_custom_object = Mock(return_value={"status": "created"})
        ...
        def _validate_experiment(self, experiment: Experiment) -> bool:
            ...
            if experiment.duration_hours <= 0 or experiment.duration_hours > 168:  # Max 1 week
        ...
        def update_traffic_split(self, experiment_id: str, new_split: Dict[str, int]) -> bool:
            ...
            try:
                ...
                self.k8s_client.get_namespaced_custom_object = Mock(
                    return_value=self.active_experiments[experiment_id]["virtual_service"]
                )
                self.k8s_client.patch_namespaced_custom_object = Mock(return_value={"status": "updated"})
                
                # Update stored experiment
                self.active_experiments[experiment_id]["experiment"].traffic_split = new_split
        ...
        def cleanup_experiment(self, experiment_id: str) -> bool:
            ...
            try:
                ...
                # Simulate k8s API call
                self.k8s_client.delete_namespaced_custom_object = Mock(return_value={"status": "deleted"})
        ...
        def get_experiment_status(self, experiment_id: str) -> Dict:
            ...
            for variant, weight in experiment.traffic_split.items():
                variant_requests = int(status["metrics"]["total_requests"] * (weight / 100))
        ...
        def analyze_experiment_results(self, experiment_id: str) -> Dict:
            ...
            for criterion, threshold in experiment.success_criteria.items():
                if criterion == "latency_improvement":
                    ...
                    baseline_latency = status["variant_performance"].get("baseline", {}).get("avg_latency_ms", 200)
                    candidate_latency = status["variant_performance"].get("candidate", {}).get("avg_latency_ms", 180)
                    improvement = (baseline_latency - candidate_latency) / baseline_latency
                    analysis["success_criteria_met"][criterion] = improvement >= threshold
                    
                elif criterion == "success_rate_maintained":
                    min_success_rate = min(
                        variant["success_rate"] for variant in status["variant_performance"].values()
                    )
                    analysis["success_criteria_met"][criterion] = min_success_rate >= threshold
            ...
            for criterion, threshold in experiment.rollback_criteria.items():
                if criterion == "latency_degradation":
                    baseline_latency = status["variant_performance"].get("baseline", {}).get("avg_latency_ms", 200)
                    candidate_latency = status["variant_performance"].get("candidate", {}).get("avg_latency_ms", 180)
                    degradation = (candidate_latency - baseline_latency) / baseline_latency
                    analysis["rollback_criteria_met"][criterion] = degradation >= threshold
                    
                elif criterion == "success_rate_drop":
                    min_success_rate = min(
                        variant["success_rate"] for variant in status["variant_performance"].values()
                    )
                    analysis["rollback_criteria_met"][criterion] = min_success_rate <= threshold
            
            # Generate recommendations
            if any(analysis["rollback_criteria_met"].values()):
                analysis["overall_health"] = "degraded"
                analysis["recommendations"].append("Consider immediate rollback due to performance degradation")
                analysis["next_actions"].append("rollback")
            elif all(analysis["success_criteria_met"].values()):
                analysis["overall_health"] = "excellent"
                analysis["recommendations"].append("Experiment shows positive results - consider increasing candidate traffic")
                analysis["next_actions"].append("increase_candidate_traffic")
            else:
                analysis["recommendations"].append("Continue monitoring - results are mixed")
...
class TestExperimentManager:
    ...
    def sample_experiment(self):
        ...
        return Experiment(
            ...
            duration_hours=12,
            success_criteria={"latency_improvement": 0.05, "success_rate_maintained": 0.97},
    ...
    def test_experiment_validation_invalid_traffic_split(self, experiment_manager):
        ...
        with pytest.raises(ValueError, match="All traffic split values must be positive"):
    ...
    def test_experiment_validation_invalid_duration(self, experiment_manager):
        ...
        with pytest.raises(ValueError, match="Experiment duration must be between 1 and 168 hours"):
    ...
    def test_virtual_service_generation(self, experiment_manager, sample_experiment):
        ...
        virtual_service = experiment_manager._generate_virtual_service(sample_experiment)
    ...
    def test_update_traffic_split_success(self, experiment_manager, sample_experiment):
        ...
        result = experiment_manager.update_traffic_split(sample_experiment.id, new_split)
        
        assert result is True
        updated_experiment = experiment_manager.active_experiments[sample_experiment.id]["experiment"]
    ...
    def test_update_traffic_split_invalid(self, experiment_manager, sample_experiment):
        ...
        result = experiment_manager.update_traffic_split(sample_experiment.id, invalid_split)
    ...
    def test_update_traffic_split_nonexistent_experiment(self, experiment_manager):
        ...
        result = experiment_manager.update_traffic_split("nonexistent-experiment", {"baseline": 100})
        assert result is False
    ...
    def test_get_experiment_status(self, experiment_manager, sample_experiment):
        ...
        required_fields = [
            "experiment_id", "status", "traffic_split", "duration_hours",
            "elapsed_hours", "metrics", "variant_performance"
        ]
    ...
    def test_analyze_experiment_results(self, experiment_manager, sample_experiment):
        ...
        required_fields = [
            "experiment_id", "analysis_timestamp", "overall_health",
            "recommendations", "success_criteria_met", "rollback_criteria_met",
            "next_actions"
        ]
    ...
    @pytest.mark.parametrize("traffic_split,expected_valid", [
    ...
    def test_traffic_split_validation(self, experiment_manager, traffic_split, expected_valid):
