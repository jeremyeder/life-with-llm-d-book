...
try:
    from chapter_06_performance.benchmarks.rdma_performance_test import \
        RDMAPerformanceTester
except ImportError:
    ...
    class RDMAPerformanceTester:
        ...
        async def test_latency(self, iterations=1000):
            """Mock latency test."""
            return {"latency_us": 2.8, "iterations": iterations, "success": True}

        async def test_inference_performance(
            self, model_endpoint="http://localhost:8080"
        ):
...
class TestRDMAPerformanceTester:
    ...
    def test_performance_comparison(self, tester):
        ...
        bandwidth_improvement = (
            rdma_results["bandwidth_gbps"] / tcp_results["bandwidth_gbps"] - 1
        ) * 100
        latency_improvement = (
            tcp_results["latency_us"] / rdma_results["latency_us"] - 1
        ) * 100
        throughput_improvement = (
            rdma_results["throughput_tokens_per_sec"]
            / tcp_results["throughput_tokens_per_sec"]
            - 1
        ) * 100
