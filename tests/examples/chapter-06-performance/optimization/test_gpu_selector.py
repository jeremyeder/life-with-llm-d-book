...
try:
    ...
    class GPUSelector:
        ...
        def calculate_model_requirements(self, model_size_params, quantization="fp16", context_length=2048):
            ...
            kv_cache_gb = context_length * 4096 * 2 * quantization_factor / 1e9  # Simplified
            
            return {
                "model_memory_gb": model_memory_gb,
                "kv_cache_gb_per_1k_tokens": kv_cache_gb * 1000 / context_length,  # Fixed calculation
                "min_memory_gb": model_memory_gb + kv_cache_gb,
                "recommended_memory_gb": model_memory_gb * 1.5 + kv_cache_gb
            }
            
        def evaluate_gpu_for_model(self, gpu_name, model_size_params, quantization="fp16", 
                                 context_length=2048, tensor_parallel=1):
            ...
            model_req = self.calculate_model_requirements(model_size_params, quantization, context_length)
            ...
            memory_efficiency = model_req["min_memory_gb"] / effective_memory if can_fit else 0
            
            # Estimate performance (simplified)
            base_throughput = gpu_spec["memory_bandwidth_tbps"] * 1000 / model_req["model_memory_gb"] if model_req["model_memory_gb"] > 0 else 0
            actual_throughput = base_throughput * tensor_parallel * 0.8  # Scaling efficiency
            ...
            return {
                ...
                "cost_per_1k_tokens": gpu_spec["cost_per_hour"] / (actual_throughput * 3.6) if actual_throughput > 0 else float('inf'),
                "tensor_parallel_required": max(1, int(model_req["min_memory_gb"] / gpu_spec["memory_gb"]) + 1),
                "recommended_tensor_parallel": tensor_parallel
            }
            
        def recommend_gpu_configuration(self, model_size_params, quantization="fp16", 
                                      budget_per_hour=None, performance_priority=True):
            """Recommend optimal GPU configuration."""
            model_req = self.calculate_model_requirements(model_size_params, quantization)
            recommendations = []
            
            for gpu_name, gpu_spec in self.gpus.items():
                # Calculate minimum tensor parallelism needed
                min_tp = max(1, int(model_req["min_memory_gb"] / gpu_spec["memory_gb"]) + 1)
                ...
                for tp in [min_tp, min_tp * 2, min_tp * 4]:
                    ...
                    if budget_per_hour and eval_result["cost_per_hour"] > budget_per_hour:
                        continue
                    ...
                    recommendations.append({
                        ...
                        "performance_score": eval_result["estimated_throughput_tokens_per_sec"],
                        "cost_per_hour": eval_result["cost_per_hour"],
                        "cost_efficiency": eval_result["estimated_throughput_tokens_per_sec"] / eval_result["cost_per_hour"],
        ...
        def compare_gpus(self, gpu_list, model_size_params, quantization="fp16"):
            ...
            for gpu_name in gpu_list:
                ...
                eval_result = self.evaluate_gpu_for_model(gpu_name, model_size_params, quantization)
...
class TestGPUSelector:
    ...
    @pytest.mark.parametrize("model_size,quantization,expected_memory", [
        (8e9, "fp16", (10, 20)),     # Llama 3.1 8B
        (70e9, "fp16", (120, 160)),  # Llama 3.1 70B
        (405e9, "fp16", (700, 900)), # Llama 3.1 405B
        (8e9, "int4", (3, 8)),       # Quantized 8B
    ])
    def test_model_requirements_calculation(self, selector, model_size, quantization, expected_memory):
    ...
    def test_gpu_evaluation_llama_70b(self, selector):
        ...
        eval_result = selector.evaluate_gpu_for_model("H100", 70e9, "fp16", tensor_parallel=1)
        assert eval_result["can_fit"] is False
        
        # Multi-GPU should fit
        eval_result = selector.evaluate_gpu_for_model("H100", 70e9, "fp16", tensor_parallel=4)
        assert eval_result["can_fit"] is True
        assert eval_result["cost_per_hour"] == selector.gpus["H100"]["cost_per_hour"] * 4
    
    def test_gpu_evaluation_llama_405b(self, selector):
        """Test GPU evaluation for Llama 3.1 405B."""
        # Should require many GPUs
        eval_result = selector.evaluate_gpu_for_model("H100", 405e9, "fp16", tensor_parallel=8)
    ...
    def test_tensor_parallelism_scaling(self, selector):
        ...
        single_gpu = selector.evaluate_gpu_for_model("H100", 8e9, "fp16", tensor_parallel=1)
        multi_gpu = selector.evaluate_gpu_for_model("H100", 8e9, "fp16", tensor_parallel=2)
        
        # Multi-GPU should have higher throughput and cost
        assert multi_gpu["estimated_throughput_tokens_per_sec"] > single_gpu["estimated_throughput_tokens_per_sec"]
    ...
    def test_quantization_impact(self, selector):
        ...
        fp16_result = selector.evaluate_gpu_for_model("H100", 70e9, "fp16", tensor_parallel=4)
        int4_result = selector.evaluate_gpu_for_model("H100", 70e9, "int4", tensor_parallel=1)
    ...
    def test_recommendations_performance_priority(self, selector):
        ...
        for i in range(len(recommendations) - 1):
            assert recommendations[i]["performance_score"] >= recommendations[i + 1]["performance_score"]
    ...
    def test_recommendations_cost_priority(self, selector):
        ...
        for i in range(len(recommendations) - 1):
            assert recommendations[i]["cost_efficiency"] >= recommendations[i + 1]["cost_efficiency"]
    ...
    def test_memory_efficiency_calculation(self, selector):
        ...
        large_model = selector.evaluate_gpu_for_model("H100", 70e9, "fp16", tensor_parallel=4)
    ...
    def test_recommendations_large_models(self, selector):
        ...
        recommendations = selector.recommend_gpu_configuration(
            model_size_params=405e9,  # 405B model
            quantization="fp16"
        )
    ...
    def test_context_length_impact(self, selector):
        ...
        assert long_context["kv_cache_gb_per_1k_tokens"] >= short_context["kv_cache_gb_per_1k_tokens"] * 0.99  # Allow for small rounding