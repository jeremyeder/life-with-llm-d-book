...
try:
    from chapter_06_performance.optimization.batch_optimizer import \
        BatchOptimizer
except ImportError:
    ...
    class BatchOptimizer:
        ...
        def calculate_memory_requirements(
            self, model_name, sequence_length=2048, quantization="fp16"
        ):
            ...
            activation_memory = (
                model_info["hidden_size"] * sequence_length * quantization_factor / 1e9
            )
        ...
        def optimize_batch_size(
            ...
            memory_req = self.calculate_memory_requirements(
                model_name, sequence_length, quantization
            )
            ...
            return {
                ...
                "memory_utilization": min(
                    1.0,
                    (base_memory + optimal_batch_size * memory_per_sequence)
                    / available_memory,
                ),
                "estimated_throughput_tokens_per_sec": optimal_batch_size
                * 50,  # Simplified estimate
                "configuration": {
                    "model": model_name,
...
class TestBatchOptimizer:
    ...
    @pytest.mark.parametrize(
        "model_name,expected_memory_range",
        [
            ("llama-3.1-8b", (10, 30)),
    ...
    def test_memory_requirements_different_models(
        self, optimizer, model_name, expected_memory_range
    ):
    ...
    def test_quantization_memory_impact(self, optimizer, quantization, expected_factor):
        ...
        fp16_memory = optimizer.calculate_memory_requirements(
            "llama-3.1-8b", 2048, "fp16"
        )
        quant_memory = optimizer.calculate_memory_requirements(
            "llama-3.1-8b", 2048, quantization
        )
    ...
    @pytest.mark.parametrize(
        "gpu_type,memory_gb",
        [("H100", 80), ("H200", 141), ("A100", 40), ("A100", 80), ("V100", 32)],
    )
    ...
    def test_throughput_estimation(self, optimizer):
        ...
        result = optimizer.optimize_batch_size(
            model_name="llama-3.1-8b", gpu_memory_gb=80, sequence_length=2048
        )
        ...
        assert (
            abs(result["estimated_throughput_tokens_per_sec"] - expected_throughput)
            < 100
        )
    ...
    def test_optimization_workflow(self, optimizer):
        ...
        config = optimizer.generate_vllm_config(
            "llama-3.1-8b", optimization_result["optimal_batch_size"]
        )
