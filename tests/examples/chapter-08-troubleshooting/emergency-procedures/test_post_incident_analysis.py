...
try:
    from chapter_08_troubleshooting.emergency_procedures.post_incident_analysis import PostIncidentAnalyzer
except ImportError:
    ...
    class PostIncidentAnalyzer:
        def __init__(self, incident_start: str, incident_end: str):
            self.start_time = datetime.fromisoformat(incident_start.replace('Z', '+00:00'))
        ...
        def analyze_logs(self):
            ...
            analysis_result = {
                ...
                "error_rate_per_minute": len(errors) / duration_minutes if duration_minutes > 0 else 0,
        ...
        def generate_recommendations(self):
            ...
            if "logs" in self.analysis_results:
                ...
                if logs_analysis.get("error_count", 0) > 10:
                    recommendations.append("Implement proactive error monitoring and alerting")
                
                error_patterns = logs_analysis.get("error_patterns", {})
                if error_patterns.get("CUDA", 0) > 0:
                    recommendations.append("Optimize GPU memory management and add CUDA error recovery")
                if error_patterns.get("Memory", 0) > 0:
                    recommendations.append("Implement memory leak detection and automatic cleanup")
                if error_patterns.get("Network", 0) > 0:
                    recommendations.append("Add network retry logic and connection pooling")
            ...
            if "metrics" in self.analysis_results:
                ...
                if gpu_util.get("above_threshold_percent", 0) > 80:
                    recommendations.append("Implement GPU load balancing and auto-scaling")
                
                memory = metrics_analysis.get("memory_usage", {})
                if memory.get("memory_leak_detected", False):
                    recommendations.append("Add automatic memory leak detection and mitigation")
                
                latency = metrics_analysis.get("request_latency", {})
                if latency.get("timeout_rate", 0) > 5:
                    recommendations.append("Optimize request timeout handling and implement graceful degradation")
            ...
            if "infrastructure" in self.analysis_results:
                ...
                if k8s_events.get("oom_kills", 0) > 0:
                    recommendations.append("Increase memory limits and implement memory pressure handling")
                if k8s_events.get("node_failures", 0) > 0:
                    recommendations.append("Implement node failure detection and automatic workload migration")
        ...
        def generate_comprehensive_report(self):
            ...
            report = {
                ...
                "executive_summary": {
                    ...
                    "services_affected": ["llm-inference", "model-api", "user-facing-chat"],
                    "estimated_user_impact": "85% of requests failed or experienced high latency",
                    "recovery_time_minutes": self.analysis_results.get("metrics", {}).get("throughput", {}).get("recovery_time_minutes", 0)
                },
        ...
        def _create_error_timeline(self, log_entries):
            ...
            for entry in log_entries:
                try:
                    ...
                    description = " ".join(entry.split()[2:])  # Remove timestamp and level
...
class TestPostIncidentAnalyzer:
    ...
    def test_log_analysis(self, analyzer):
        ...
        required_fields = [
            "incident_duration_minutes", "total_log_entries", "error_count",
            "warning_count", "error_patterns", "error_rate_per_minute",
            "top_error_sources", "timeline"
        ]
    ...
    def test_metrics_analysis(self, analyzer):
        ...
        required_sections = [
            "gpu_utilization", "memory_usage", "request_latency",
            "throughput", "error_rate", "correlations"
        ]
        ...
        assert memory_metrics["min_gb"] <= memory_metrics["avg_gb"] <= memory_metrics["max_gb"]
        assert isinstance(memory_metrics["memory_leak_detected"], bool)
        
        # Verify latency metrics
        latency_metrics = result["request_latency"]
        assert latency_metrics["min_ms"] <= latency_metrics["avg_ms"] <= latency_metrics["max_ms"]
        assert latency_metrics["avg_ms"] <= latency_metrics["p95_ms"] <= latency_metrics["p99_ms"]
    ...
    def test_infrastructure_analysis(self, analyzer):
        ...
        required_sections = [
            "kubernetes_events", "resource_utilization", "service_health"
        ]
    ...
    def test_timeline_generation(self, analyzer):
        ...
        for event in timeline:
            required_fields = ["timestamp", "event_type", "description", "severity", "source"]
            for field in required_fields:
    ...
    def test_comprehensive_report_generation(self, analyzer):
        ...
        required_sections = [
            "incident_metadata", "executive_summary", "detailed_analysis",
            "timeline", "root_cause_analysis", "lessons_learned", "action_items"
        ]
    ...
    def test_lessons_learned_extraction(self, analyzer):
        ...
        assert any(keyword in lessons_text for keyword in ["monitoring", "degradation", "limits", "circuit"])
    ...
    def test_action_items_creation(self, analyzer):
        ...
        for item in action_items:
            required_fields = ["action", "owner", "priority", "due_date", "estimated_effort"]
            for field in required_fields:
    ...
    def test_different_incident_durations(self, start_time, end_time, expected_duration):
