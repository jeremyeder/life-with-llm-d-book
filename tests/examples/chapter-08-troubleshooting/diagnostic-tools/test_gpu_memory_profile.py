...
try:
    from chapter_08_troubleshooting.diagnostic_tools.gpu_memory_profile import analyze_gpu_memory
except ImportError:
    ...
    def analyze_gpu_memory():
        ...
        return {
            ...
            "num_gpus": num_gpus,
            "gpu_data": gpu_data,
            "total_gpu_memory_gb": sum(gpu["memory_total_mb"] for gpu in gpu_data) / 1024,
...
class TestGPUMemoryProfiler:
    ...
    @patch('sys.modules', {'torch': MockTorch(), 'psutil': MockPsutil(), 'GPUtil': MockGPUtil()})
    def test_gpu_memory_analysis_execution(self):
        ...
        required_fields = [
            "system_memory_percent", "num_gpus", "gpu_data",
            "total_gpu_memory_gb", "total_allocated_gb", "total_cached_gb"
        ]
    ...
    def test_gpu_data_structure(self):
        ...
        for gpu in gpu_data:
            required_fields = [
                "gpu_id", "name", "allocated_gb", "cached_gb",
                "gpu_load_percent", "memory_used_mb", "memory_total_mb",
                "temperature_c", "memory_utilization_percent", "available_memory_gb"
            ]
    ...
    def test_memory_calculations(self):
        ...
        expected_total_memory = sum(gpu["memory_total_mb"] for gpu in result["gpu_data"]) / 1024
    ...
    def test_average_calculations(self):
        ...
        if "avg_gpu_load" in result:
            ...
            expected_avg_load = sum(gpu["gpu_load_percent"] for gpu in result["gpu_data"]) / result["num_gpus"]
            assert abs(result["avg_gpu_load"] - expected_avg_load) < 0.01
        
        if "avg_temperature" in result:
            # Test average temperature
            expected_avg_temp = sum(gpu["temperature_c"] for gpu in result["gpu_data"]) / result["num_gpus"]
    ...
    def test_memory_fragmentation_detection(self):
        ...
        for gpu in result["gpu_data"]:
            ...
            assert abs(actual_used_gb - expected_used_gb) <= 10.0  # 10GB tolerance for mock data
    ...
    def test_performance_metrics_collection(self):
        ...
        for gpu in result["gpu_data"]:
            ...
            elif gpu["memory_utilization_percent"] > 80 and gpu["gpu_load_percent"] < 50:
    ...
    def test_memory_efficiency_recommendations(self):
        ...
        for gpu in result["gpu_data"]:
            ...
            if gpu["memory_utilization_percent"] < 30:
                recommendations.append(f"GPU {gpu['gpu_id']}: Consider consolidating workloads")
            
            # High utilization warning
            elif gpu["memory_utilization_percent"] > 90:
                recommendations.append(f"GPU {gpu['gpu_id']}: Risk of OOM, consider reducing batch size")
            
            # High cache ratio
            cache_ratio = gpu["cached_gb"] / gpu["allocated_gb"] if gpu["allocated_gb"] > 0 else 0
            if cache_ratio > 0.3:
                recommendations.append(f"GPU {gpu['gpu_id']}: High cache usage, consider manual cache clearing")
