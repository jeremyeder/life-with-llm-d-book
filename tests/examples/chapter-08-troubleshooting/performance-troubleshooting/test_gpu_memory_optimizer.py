...
try:
    from chapter_08_troubleshooting.performance_troubleshooting.gpu_memory_optimizer import GPUMemoryOptimizer
except ImportError:
    ...
    class GPUMemoryOptimizer:
        ...
        def optimize_memory_allocation(self, target_utilization=0.85):
            ...
            if current_util > target_utilization:
                ...
                freed_gb = (current_util - target_utilization) * current_stats["total_memory_gb"]
            else:
        ...
        def apply_memory_optimizations(self, optimizations=None):
            ...
            if optimizations is None:
                optimizations = ["clear_cache", "reduce_fragmentation", "optimize_allocation"]
            ...
            return {
                ...
                "summary": {
                    ...
                    "optimizations_applied": len([r for r in results.values() if r.get("applied", False)]),
...
class TestGPUMemoryOptimizer:
    ...
    def test_memory_stats_collection(self, optimizer):
        ...
        required_fields = [
            "device_id", "total_memory_gb", "allocated_memory_gb",
            "cached_memory_gb", "free_memory_gb", "memory_utilization",
            "fragmentation_ratio", "largest_free_block_gb"
        ]
    ...
    def test_memory_allocation_optimization(self, optimizer):
        ...
        assert result["action_taken"] in ["reduce_allocation", "maintain_current", "increase_allocation"]
    ...
    def test_memory_usage_profiling(self, optimizer):
        ...
        required_metrics = [
            "min_usage_gb", "max_usage_gb", "avg_usage_gb", "std_deviation_gb"
        ]
        ...
        assert profile["min_usage_gb"] <= profile["avg_usage_gb"] <= profile["max_usage_gb"]
    ...
    def test_memory_report_generation(self, optimizer):
        ...
        required_sections = [
            "device_info", "current_status", "optimization_history",
            "health_assessment", "performance_metrics"
        ]
        ...
        assert health["overall_health"] in ["excellent", "good", "fair", "poor", "critical"]
    ...
    def test_optimization_performance_impact(self, optimizer):
        ...
        for opt_name, opt_result in result["optimization_results"].items():
            if opt_result.get("applied", False):
                ...
                assert has_positive_impact, f"Optimization {opt_name} should have positive impact"