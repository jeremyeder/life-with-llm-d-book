...
try:
    from chapter_08_troubleshooting.performance_troubleshooting.load_test import \
        LoadTester
except ImportError:
    ...
    class LoadTester:
        ...
        async def make_request(self, session, request_id: int):
            ...
            if random.random() < 0.05:
                self.errors += 1
                return {"request_id": request_id, "error": "Connection timeout"}
        ...
        async def run_test(self):
            ...
            return {
                ...
                "throughput_req_per_sec": (
                    self.successful_requests / total_time if total_time > 0 else 0
                ),
        ...
        def get_detailed_metrics(self):
            ...
            return {
                "request_metrics": {
                    ...
                    "successful_requests": self.successful_requests,
                    "failed_requests": self.errors,
                    "success_rate": (self.successful_requests / self.num_requests)
                    * 100,
                ...
                "throughput_metrics": {
                    ...
                    "peak_throughput_req_per_sec": self.concurrency
                    / (np.min(latencies_array) if len(latencies_array) > 0 else 1),
                    "concurrency_level": self.concurrency,
                },
                "token_metrics": {
                    "total_tokens_generated": sum(
                        r.get("tokens", 0) for r in self.results if "tokens" in r
                    ),
                    ...
                    "tokens_per_second": (
                        sum(r.get("tokens", 0) for r in self.results)
                        / sum(self.latencies)
                        if self.latencies
                        else 0
                    ),
        ...
        def analyze_performance_bottlenecks(self):
            ...
            # Analyze throughput
            avg_throughput = (
                len(self.latencies) / sum(self.latencies) if self.latencies else 0
            )
            ...
            if throughput_efficiency < 70:
                bottlenecks.append(
                    f"Low throughput efficiency: {throughput_efficiency:.1f}%"
                )
                recommendations.append(
                    "Consider increasing concurrency or optimizing request processing"
                )
            ...
            return {
                ...
                "performance_analysis": {
                    "latency_variance_ratio": (
                        p95_latency / mean_latency if mean_latency > 0 else 0
                    ),
                    ...
                    "concurrency_utilization": (
                        min(
                            100,
                            (avg_throughput * mean_latency / self.concurrency) * 100,
                        )
...
class TestLoadTester:
    ...
    async def test_load_test_execution(self, load_tester):
        ...
        total_processed = (
            test_results["successful_requests"] + test_results["failed_requests"]
        )
    ...
    async def test_different_load_configurations(self, num_requests, concurrency):
        ...
        if test_results["total_time"] > 0:
            expected_throughput = (
                test_results["successful_requests"] / test_results["total_time"]
            )
            assert (
                abs(test_results["throughput_req_per_sec"] - expected_throughput) < 0.01
            )
    ...
    async def test_request_payload_customization(self, load_tester):
        ...
        async def capture_payload_make_request(session, request_id):
            ...
            payloads_sent.append(
                {"prompt": "Once upon a time", "max_tokens": 100, "temperature": 0.7}
            )
    ...
    async def test_performance_regression_detection(self, load_tester):
        ...
        # Compare performance
        if (
            baseline_results["successful_requests"] > 0
            and degraded_results["successful_requests"] > 0
        ):
            ...
            throughput_ratio = (
                degraded_throughput / baseline_throughput
