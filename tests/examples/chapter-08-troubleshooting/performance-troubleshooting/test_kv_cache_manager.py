...
try:
    from chapter_08_troubleshooting.performance_troubleshooting.kv_cache_manager import \
        KVCacheManager
except ImportError:
    ...
    class KVCacheManager:
        def __init__(
            ...
            self.cache_lens = MockTensor(
                (max_batch_size,), dtype="int32", device="cuda"
            )
        ...
        def update_cache(
            self, layer_idx: int, key_value, batch_idx: int, seq_start: int
        ):
            """Update cache with new key-value pairs"""
            if layer_idx >= self.num_layers:
                raise ValueError(
                    f"Layer index {layer_idx} exceeds maximum {self.num_layers}"
                )
            if batch_idx >= self.max_batch_size:
                raise ValueError(
                    f"Batch index {batch_idx} exceeds maximum {self.max_batch_size}"
                )
            ...
            if seq_start + seq_len > self.max_seq_len:
                raise ValueError(
                    f"Sequence position {seq_start + seq_len} exceeds maximum {self.max_seq_len}"
                )
        ...
        def get_cache_utilization(self):
            ...
            return {
                "utilization_percent": (
                    (active_entries / total_entries) * 100 if total_entries > 0 else 0
                ),
        ...
        def get_memory_stats(self):
            ...
            return {
                ...
                "used_memory_gb": (used_entries / self.max_batch_size)
                * self.memory_usage_gb,
                "free_memory_gb": (
                    (self.max_batch_size - used_entries) / self.max_batch_size
                )
                * self.memory_usage_gb,
        ...
        def prefetch_cache(self, batch_size: int, expected_seq_len: int):
            ...
            return {
                ...
                "memory_prepared_gb": (batch_size / self.max_batch_size)
                * self.memory_usage_gb,
...
class TestKVCacheManager:
    ...
    def cache_manager(self):
        ...
        return KVCacheManager(
            max_batch_size=8, max_seq_len=1024, num_heads=16, head_dim=64, num_layers=12
        )
    ...
    def test_cache_slice_creation(self, cache_manager):
        ...
        full_slice = cache_manager.get_cache_slice(
            batch_size=cache_manager.max_batch_size, seq_len=cache_manager.max_seq_len
        )
    ...
    def test_cache_update_basic(self, cache_manager):
        ...
        result = cache_manager.update_cache(
            layer_idx=0, key_value=(key, value), batch_idx=0, seq_start=0
        )
    ...
    def test_memory_statistics(self, cache_manager):
        ...
        assert (
            stats["used_memory_gb"] + stats["free_memory_gb"]
    ...
    @pytest.mark.parametrize(
        "batch_size,seq_len,num_heads,head_dim,num_layers",
        [
            (4, 512, 8, 32, 6),
            (16, 2048, 32, 128, 24),
            (32, 4096, 64, 256, 48),
    ...
    def test_different_cache_configurations(
        self, batch_size, seq_len, num_heads, head_dim, num_layers
    ):
    ...
    def test_multi_layer_cache_updates(self, cache_manager):
        ...
        for layer_idx in range(cache_manager.num_layers):
            result = cache_manager.update_cache(
                layer_idx, (key, value), batch_idx, seq_start
            )
