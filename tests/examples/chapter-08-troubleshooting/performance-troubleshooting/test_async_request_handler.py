...
try:
    from chapter_08_troubleshooting.performance_troubleshooting.async_request_handler import AsyncModelServer
except ImportError:
    ...
    class AsyncModelServer:
        ...
        def get_metrics(self):
            ...
            return {
                ...
                "is_running": self.batch_processor_task is not None and not self.batch_processor_task.done(),
...
class TestAsyncModelServer:
    ...
    def mock_model(self):
        ...
        model.generate = Mock(return_value=[
            Mock(spec=[]),  # Mock tensor-like objects
            Mock(spec=[])
        ])
        return model
    ...
    async def test_server_start_stop(self, server):
        ...
        # Stop server
        await server.stop()
        assert server.batch_processor_task.cancelled() or server.batch_processor_task.done()
