...
try:
    from cost_optimization.llm_cost_calculator import (CloudProvider,
                                                       DeploymentConfig,
                                                       GPUSpecs, GPUType,
                                                       LLMCostCalculator,
                                                       ModelSpecs)
...
class TestLLMCostCalculator:
    ...
    def test_calculate_memory_requirements_large_model(self, calculator, large_model):
        ...
        single_batch_model = ModelSpecs(
            name="large-model", parameters=70_000_000_000, batch_size=1
        )
    ...
    def test_calculate_gpu_requirements(self, calculator, small_model):
        ...
        assert gpu_req["recommended_gpus"] == max(
            gpu_req["required_gpus_throughput"], gpu_req["required_gpus_memory"]
        )
    ...
    def test_calculate_gpu_requirements_standard_models(self, calculator):
        ...
        assert (
            gpu_req["tokens_per_second_per_gpu"] == 150
        )  # From base_tokens_per_second
    ...
    def test_calculate_deployment_cost(self, calculator, small_model):
        ...
        config = DeploymentConfig(
            model=small_model, gpu=gpu_specs, replicas=2, utilization_target=0.7
        )
    ...
    @pytest.mark.parametrize(
        "provider,gpu_type,expected_cost",
        [
    ...
    def test_memory_scaling_with_context(self, calculator):
        ...
        model_short = ModelSpecs(
            name="test", parameters=1_000_000_000, context_length=1024
        )
        model_long = ModelSpecs(
            name="test", parameters=1_000_000_000, context_length=8192
        )
    ...
    def test_gpu_requirements_high_throughput(self, calculator, small_model):
        ...
        assert (
            high_req["required_gpus_throughput"] > low_req["required_gpus_throughput"]
        )
    ...
    def test_storage_cost_calculation(self, calculator, small_model):
        ...
        model_size_gb = (
            small_model.parameters * small_model.memory_per_param_bytes
        ) / (1024**3)
...
class TestCostOptimizationScenarios:
    ...
    def test_cost_breakdown_accuracy(self, calculator):
        ...
        config = DeploymentConfig(model=model, gpu=gpu_specs, replicas=2)
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_extreme_throughput_handling(self, calculator):
        ...
        model = calculator.standard_models["llama-3.1-8b"]
        gpu_req = calculator.calculate_gpu_requirements(
            model, target_throughput_rps=1000
        )
