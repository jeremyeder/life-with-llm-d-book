...
try:
    ...
    class QuantizationOptimizer:
        ...
        def analyze_quantization_options(self, model_name, available_gpu_memory_gb, performance_threshold_pct=95.0):
            options = []
...
class TestQuantizationOptimizer:
    ...
    def test_analyze_quantization_options_basic(self, optimizer):
        ...
        if len(options) > 1:
            assert options[0]["estimated_monthly_savings_usd"] >= options[1]["estimated_monthly_savings_usd"]
    ...
    def test_implementation_complexity_mapping(self, optimizer):
        ...
        assert optimizer._get_implementation_complexity(QuantizationType.INT8) == "Medium"
        assert optimizer._get_implementation_complexity(QuantizationType.INT4) == "High"
        assert optimizer._get_implementation_complexity(QuantizationType.MIXED_PRECISION) == "Medium"
        assert optimizer._get_implementation_complexity(QuantizationType.DYNAMIC) == "High"
...
class TestCostOptimizationScenarios:
    ...
    def test_cost_vs_performance_tradeoffs(self, optimizer):
        ...
        options_by_perf = sorted(options, key=lambda x: x["performance_retention_pct"], reverse=True)
        
        # Sort by cost savings (descending)
        options_by_cost = sorted(options, key=lambda x: x["cost_reduction_pct"], reverse=True)
        ...
        if len(options) > 1:
            assert highest_perf["quantization_type"] != highest_cost_saving["quantization_type"]
    ...
    def test_model_specific_optimization_strategies(self, optimizer):
        ...
        for model in models:
            ...
            if options:
                total_savings = sum(opt["estimated_monthly_savings_usd"] for opt in options)
    ...
    def test_implementation_complexity_vs_savings(self, optimizer):
        ...
        if "Low" in complexity_savings and "High" in complexity_savings:
            avg_low_savings = sum(complexity_savings["Low"]) / len(complexity_savings["Low"])
            avg_high_savings = sum(complexity_savings["High"]) / len(complexity_savings["High"])
