...
try:
    from cost_optimization.dynamic_router import (LLMDDynamicRouter,
                                                  RequestComplexity)
except ImportError:
...
class TestLLMDDynamicRouter:
    ...
    def test_analyze_code_request(self, router):
        ...
        assert (
            complexity.expected_output_length > complexity.prompt_length * 3
        )  # "write" increases output
    ...
    def test_analyze_math_request(self, router):
        ...
        assert (
            complexity.expected_output_length == complexity.prompt_length * 2
        )  # Default multiplier
...
class TestCostOptimizationScenarios:
    ...
    async def test_cost_savings_analysis(self, router):
        ...
        for prompt, slo in test_requests:
            ...
            expensive_cost = (
                expensive_cost_per_token * metadata["complexity"].expected_output_length
            )
        ...
        savings_ratio = (
            total_expensive_cost - total_optimized_cost
        ) / total_expensive_cost
