...
class LLMCostCalculator:
    ...
    def calculate_memory_requirements(self, model: ModelSpecs) -> Dict[str, float]:
        ...
        kv_cache_gb = (
            kv_cache_per_token_bytes * model.context_length * model.batch_size
        ) / (1024**3)
        ...
        total_memory_gb = (
            model_memory_gb + kv_cache_gb + activation_memory_gb + system_overhead_gb
        )
    ...
    def calculate_gpu_requirements(
        self, model: ModelSpecs, target_throughput_rps: float
    ) -> Dict:
        ...
        required_gpus_throughput = math.ceil(
            target_throughput_rps * 50 / tokens_per_sec
        )  # Assume 50 tokens per request
        ...
        if memory_req["total_memory_gb"] > suitable_gpu_memory:
            ...
            required_gpus_memory = math.ceil(
                memory_req["total_memory_gb"] / suitable_gpu_memory
            )
        else:
    ...
    def calculate_deployment_cost(
        self, config: DeploymentConfig, hours_per_month: int = 730
    ) -> Dict:
        ...
        total_monthly_cost = (
            monthly_gpu_cost + storage_cost + network_cost + management_overhead
        )

        # Cost per request calculation
        estimated_requests_per_month = self.estimate_monthly_requests(config)
        cost_per_request = (
            total_monthly_cost / estimated_requests_per_month
            if estimated_requests_per_month > 0
            else 0
        )
    ...
    def compare_providers(self, model_name: str, target_rps: float) -> Dict:
        ...
        for provider in CloudProvider:
            ...
            for gpu_type in self.gpu_pricing[provider]:
                ...
                if (
                    gpu_req["recommended_gpu_memory"] > 40
                    and gpu_type != GPUType.A100_80GB
                ):
                    continue
                if gpu_req["recommended_gpu_memory"] <= 40 and gpu_type not in [
                    GPUType.A100_40GB,
                    GPUType.V100_32GB,
                ]:
                    continue
        ...
        sorted_comparison = dict(
            sorted(comparison.items(), key=lambda x: x[1]["monthly_cost"])
        )
...
def main():
    ...
    print(
        f"  Memory Requirements: {gpu_req['memory_requirements']['total_memory_gb']:.1f} GB"
    )
    ...
    for config_name, costs in list(comparison.items())[:5]:  # Top 5 cheapest
        print(
            f"  {costs['provider']} ({costs['gpu_type']}): ${costs['monthly_cost']:.0f}/month, ${costs['cost_per_request']:.4f}/request"
        )
    ...
    if comparison:
        ...
        config = DeploymentConfig(
            model=model, gpu=gpu_specs, replicas=top_choice["gpu_count"]
        )
        ...
        for cost_type, amount in detailed_costs["monthly_costs"].items():
            if cost_type != "total":
                ...
                print(
                    f"    {cost_type.replace('_', ' ').title()}: ${amount:.2f} ({pct:.1f}%)"
                )
